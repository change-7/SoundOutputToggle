...
`SoundOutputToggle.app` 아이콘은 실행할 때마다 바뀌지 않는 정적 아이콘입니다.

토글 앱은 파란색 기본 아이콘, 설정 앱은 톱니바퀴가 들어간 설정용 아이콘으로 구분됩니다.

`Show switching HUD`를 켜면 출력 전환 직후 macOS 기본 볼륨 HUD와 비슷한 어두운 반투명 오버레이가 짧게 표시됩니다. 여러 모니터를 연결한 경우 모든 화면에 함께 표시되므로 현재 전환된 출력 기기를 바로 확인할 수 있어 권장합니다.
